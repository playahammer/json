...
/* Internal functions */
static int json_comments_skip(uint32_t *col, uint32_t *row, char *json_content,
...
#define match_digit(reader)               \
  (match(reader, '0') == MATCH_SUCCESS || \
   match(reader, '1') == MATCH_SUCCESS || \
   match(reader, '2') == MATCH_SUCCESS || \
   match(reader, '3') == MATCH_SUCCESS || \
   match(reader, '4') == MATCH_SUCCESS || \
   match(reader, '5') == MATCH_SUCCESS || \
   match(reader, '6') == MATCH_SUCCESS || \
   match(reader, '7') == MATCH_SUCCESS || \
   match(reader, '8') == MATCH_SUCCESS || match(reader, '9') == MATCH_SUCCESS)
#define match_alpha(reader)               \
  (match(reader, 'a') == MATCH_SUCCESS || \
   match(reader, 'b') == MATCH_SUCCESS || \
   match(reader, 'c') == MATCH_SUCCESS || \
   match(reader, 'd') == MATCH_SUCCESS || \
   match(reader, 'e') == MATCH_SUCCESS || \
   match(reader, 'f') == MATCH_SUCCESS || \
   match(reader, 'g') == MATCH_SUCCESS || \
   match(reader, 'h') == MATCH_SUCCESS || \
   match(reader, 'i') == MATCH_SUCCESS || \
   match(reader, 'j') == MATCH_SUCCESS || \
   match(reader, 'k') == MATCH_SUCCESS || \
   match(reader, 'l') == MATCH_SUCCESS || \
   match(reader, 'm') == MATCH_SUCCESS || \
   match(reader, 'n') == MATCH_SUCCESS || \
   match(reader, 'o') == MATCH_SUCCESS || \
   match(reader, 'p') == MATCH_SUCCESS || \
   match(reader, 'q') == MATCH_SUCCESS || \
   match(reader, 'r') == MATCH_SUCCESS || \
   match(reader, 's') == MATCH_SUCCESS || \
   match(reader, 't') == MATCH_SUCCESS || \
   match(reader, 'u') == MATCH_SUCCESS || \
   match(reader, 'v') == MATCH_SUCCESS || \
   match(reader, 'w') == MATCH_SUCCESS || \
   match(reader, 'x') == MATCH_SUCCESS || \
   match(reader, 'y') == MATCH_SUCCESS || \
   match(reader, 'z') == MATCH_SUCCESS || \
   match(reader, 'A') == MATCH_SUCCESS || \
   match(reader, 'B') == MATCH_SUCCESS || \
   match(reader, 'C') == MATCH_SUCCESS || \
   match(reader, 'D') == MATCH_SUCCESS || \
...
static int json_build(struct json_obj *obj, char *json_content, uint32_t size,
 ...
 * Example: LOAD_OBJECT(LOAD_STRING("123"), LOAD_ARRAY(LOAD_NUMBER(123),
 * LOAD_NULL, LOAD_TRUE, END), LOAD_STRING("234"), LOAD_OBJECT(END), END) Ouput:
 * {"123":[123, null, true], 234: {}}
 */
/* Wrapper functions */
...
#define LOAD_NUMBER(num)                                                \
  load_single_value((long long)num == num ? json_utils_long2str(num)    \
                                          : json_utils_double2str(num), \
                    V_NUMBER)
